...
notebook = {
    ...
    'cells': [
        ...
        },
        {
            ...
            'execution_count': None,
            'source': [
                'def filter_short_tracks(nodes_df, edges_df, T, min_len=3):\n',
                '    G = nx.DiGraph()\n',
                '    for _, node in nodes_df.iterrows():\n',
                '        G.add_node(int(node[\'node_id\']), t=int(node[\'t\']), z=node[\'z\'], y=node[\'y\'], x=node[\'x\'])\n',
                ...
                '        times = [G.nodes[n][\'t\'] for n in comp_nodes]\n',
                '        t_min = min(times)\n',
                '        t_max = max(times)\n',
                '        track_len = t_max - t_min + 1\n',
                '        has_division = any(G.out_degree(n) >= 2 for n in comp_nodes)\n',
                '        if track_len <= 2 and t_min > 0 and t_max < T - 1 and not has_division:\n',
                '            for n in comp_nodes:\n',
                '                nodes_to_remove.add(n)\n',
                '    filtered_nodes_df = nodes_df[~nodes_df[\'node_id\'].isin(nodes_to_remove)].copy()\n',
                '    filtered_edges_df = edges_df[\n',
                '        (~edges_df[\'source_id\'].isin(nodes_to_remove)) &\n',
                '        (~edges_df[\'target_id\'].isin(nodes_to_remove))\n',
                '    ].copy()\n',
                '    return filtered_nodes_df, filtered_edges_df\n'
            ]
        },
        {
            'cell_type': 'code',
            ...
            'source': [
                ...
                '            pred_nodes, pred_edges = filter_short_tracks(pred_nodes, pred_edges, T, min_len=3)\n',
                '            \n',
